...
--SUBSTR 문자열 자르기
SELECT FIRST_NAME, SUBSTR(FIRST_NAME, 3), SUBSTR(FIRST_NAME, 3, 2) FROM EMPLOYEES; --3미만 절삭(3이상 출력), 3번째 위치에서 2개 자름
...
SELECT REPLACE('서울 대구 대전 부산 찍고', ' ', '') FROM DUAL; --공백삭제
...
SELECT CONCAT( '(02)' , SUBSTR(phone_number, 4) ) PHONE_NUMBER FROM EMPLOYEES; --함수 중첩가능
--
--
...
SELECT FIRST_NAME,
        RPAD( SUBSTR(FIRST_NAME, 1, 3) , LENGTH(FIRST_NAME) , '*') AS 이름,
