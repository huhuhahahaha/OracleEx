...
-- 시퀀스 사용방법 2개
SELECT DEPTS_SEQ.CURRVAL FROM DUAL; --현재 시퀀스 확인( NEXTVAL 이 선행되어야함)
...
--만약 시퀀스를 초기화 해야 한다면?
--시퀀스의 증가값을 -음수로 만들어서 초기화 인것처럼 쓸 수는 있습니다
...
------------------------------
--시퀀스 응용( 나중에 테이블을 설계할때 데이터가 엄청 많다면 PK에 시퀀스 사용고려)
--문자열 PK (년도값-일련번호)
CREATE TABLE DEPTS2 (
