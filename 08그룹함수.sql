...
-- GROUP화 시킨 COLUMN만 SELECT 구문에 적을수 있다.
-- 2개 이상의 그룹화(하위 그룹)
SELECT DEPARTMENT_ID, JOB_ID,
       SUM(SALARY) AS "부서직무별 급여합",
       SUM(SALARY) AS "부서직무별 급여평균",
       COUNT(*) AS 부서인원수,
       COUNT(*) OVER() 직무별개수 --COUNT(*) OVER() 사용하면 총 행의 개수를 출력할수 있음
FROM EMPLOYEES
...
SELECT DEPARTMENT_ID,
       AVG(SALARY)
FROM EMPLOYEES
WHERE AVG(SALARY)>=5000 -- WHERE는 그룹하기전에 조건을 적어야함
-- 그룹의 조건을 적는 곳은 HAVING 이라고 따로 있음
GROUP BY DEPARTMENT_ID;
-----------------------------
-- HAVING 절 - 그룹바이의 조건
...
--
-- 부서아이디가 NULL이 아닌 데이터 중에서, 입사년도가 05인 사람들의 급여평균,급여합
-- 평균급여는 5000 이상만, 부서아이디로 내림차순
SELECT DEPARTMENT_ID, AVG(SALARY) AS 평균급여, SUM(SALARY)AS 급여합
...
--------------------------------------------
-- 시험대비용
-- ROLLUP - GROUP BY 절과 사용되고, 상위그룹의 합계,토탈등을 구함
SELECT   DEPARTMENT_ID,
         SUM(SALARY),
         AVG(SALARY)
FROM EMPLOYEES
...
-- CUBE - 롤업+ 서브그룹의 총계가 추가됨
SELECT DEPARTMENT_ID, -- 주 그룹
       JOB_ID,        -- 서브그룹
       SUM(SALARY),
       AVG(SALARY)
FROM EMPLOYEES
...
-- GROUPING - 그룹 바이로 만들어진 경우는 0반환, 롤업 또는 큐브로 만들어진 행인 경우는 1을 반환
...
FROM EMPLOYEES
GROUP BY JOB_ID
ORDER BY 월급평균 DESC, JOB_ID DESC;
...
--직업별 월급합, 총합계를 출력하세요
--
SELECT DECODE(GROUPING(JOB_ID),1,'합계',JOB_ID), SUM(SALARY)AS 월급합
FROM EMPLOYEES
...
SELECT DECODE(GROUPING(DEPARTMENT_ID),1,'합계',DEPARTMENT_ID)AS 부서,
       DECODE(GROUPING(JOB_ID),1,'소계',JOB_ID)AS 직업,
       COUNT(*) AS TOTAL,
       SUM(SALARY) AS SUM
FROM EMPLOYEES
GROUP BY ROLLUP(DEPARTMENT_ID, JOB_ID)
